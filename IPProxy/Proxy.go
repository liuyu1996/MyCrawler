...
func checkProxyIP(proxyIp string, i int) {
	req, _ := http.NewRequest("GET", "http://httpbin.org/get", nil) //验证代理是否可用
	...
	if resp != nil && resp.StatusCode == 200 {
		...
		if strings.Contains(reStr, "http://httpbin.org/get") { //验证代理有没有做手脚，可能给你返回一堆广告
			useful++
			fmt.Println(proxyIp)
			ipList = append(ipList, proxyIp)
		}
...
func GetHttpClient() *http.Client {
	...
	proxy := func(_ *http.Request) (*url.URL, error) {
		return url.Parse(ipList[rand.Intn(len(ipList))])
	}
